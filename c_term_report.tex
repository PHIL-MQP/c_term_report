...
% This will be the _last_ section we ever write.
...
\section{Survey of Localization Techniques} \label{related_work}
  ...
  \begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{./images/gao_qingji_onboard_2015_results.png}
    \caption{This plot was generated by walking with a quadcopter in-hand along a 4m square. The red line is the known algorithm developed by \cite{dominik_honegger_open_2013} and the green line is the fused estimate developed by \cite{gao_qingji_onboard_2015}. Data are XY displacements in meters, where (0,0) is the starting point. }
...
\section{Experimental Results} \label{experiments}
  ...
  \subsection{Characterising Drift and Bias in the Accelerometer}
    ...
    In this section, we demonstrate that naive double integration becomes inaccurate within \SI{-999}{\second}, but that with calibration, base frame rotation, and zero velocity updates we can achieve accurate position information for \SI{-999}{\second}. % FIXME: fill in real numbers here
    % Figure \ref{fig:raw_displacement} shows the double integration of accelerometer readings in X and Y with no calibration, bias, drift, or temperature compensation, or zero velocity updates. Because double integration amplifies errors, this is not accurate for long enough to be useful.

    % TODO: figure out how how long the raw data double integration is accurate for

    %\begin{figure}[H]
      %\centering
      %\includegraphics[width=1\linewidth]{./images/raw-displacement.png}
      %\caption{The result of double integrating the raw accelerometer measurements in X and Y.}
      %\label{fig:raw_displacement}
    %\end{figure}

    \subsubsection{Measuring the drift and bias in the accelerometer} \label{section:drift_bias}
      % we can do the same for the nypro stuff, showing how drift effects velocity and position as you integrate/double integrate
  ...
  \subsection{Detecting ArUco Tags at a Distance}
     ...
     % TODO: run this test. Put up tag flat on wall and find out how far away we can see tags at various resolutions
  ...
  \subsection{Rate of position estimates from ArUco Tags}
	...
	\subsection{Benchmarking Aruco and MarkerMapper with VICON Motion Capture}

		Using the Vicon motion capture system, data from the PlayStation Eye Camera and Logitech C920 were recorded at 480p x 60 FPS and 1080p x 30 FPS, respectively. During this test, only two cameras were used to achieve in practice the desired frames per second with both cameras (median values of 27.76 and 60, respectively). A Kobuki Turtlebot 2 base was used as a mobile platform, driven in a square and randomly around the area to simulate teleoperation during gameplay. ARUCO16h3 Tags marked with Vicon trackers were placed in a circle 3 meters in diameter around the robot. A custom dictionary was defined containing only the tags printed to avoid misdetecting tags that do not actually exist. The position of the robot was tracked during the two trials. 

		To benchmark the accuracy of ARUCO tag detection, the pose of each tag was determined offline. Using matplotlib, the distance from Tag 2 was plotted over time and compared against the distance between the two tracked objects reported by the Vicon system.

 		\begin{figure}[H]
      \centering
      \includegraphics[width=0.98\linewidth]{./images/tag_2_disp_aruco_mocap_tb.png}
      \caption{Distance from a tag placed adjacent to the origin.}
      \label{fig:dist_to_tag_two}
    \end{figure}

    Future results include statistical analysis on the rate of detection for all tags and on the accuracy of the pose estimate. Sources of error include offsets between the position of the camera and the position of the Vicon trackers on the robot and offsets between the trackers on the paper and the centers of ARUCO tags. These are approximately 10cm and 4cm respectively. In future tests, these results will also be compared against data from the IMU and Kobuku encoders.  

    To benchmark Marker Mapper, a Marker Map was generated using the Logitech C920 camera at 1080p. Several Marker Maps were built with different pose estimates. The one most closely approximating the configuration of the Map was selected for offline anaysis. 

		\begin{figure}[H]
      \centering
      \includegraphics[width=0.98\linewidth]{./images/marker_map_turtlebot_2.png}
      \caption{The blue rectagles represent markers, the red the origin, the green pose estimates of the camera during mapping.}
      \label{fig:marker_mapper_mocap_trial}
    \end{figure}

    Using this map as input, the pose estimate with respect to Tag 0 (the origin) was determined. The plot below describes the position estimate from Marker Mapper and from the Vicon System during the simulated autonomous trial. Further analysis is needed, but the below plot suggests that the global position estimate must be filtered using other sensors. When plotted over time, a reasonable position estimate can be inferred over a short peroid of time. However, Marker Mapper fails to track the robot after making a corner.

    \begin{figure}[H]
      \centering
      \includegraphics[width=0.98\linewidth]{./images/vicon_mocap_square_tb.png}
      \caption{Pose estimate from Marker Mapper in a light hue, Vicon in a darker hue. The vicon data are a tight square and the Marker Mapper data are more widely distributed. This data suggests that the position estimate is accurate over a short peroid of time, but is not suitable unless filtered.}
      \label{fig:marker_mapper_output_mocap_tb}
    \end{figure}

    Further analysis is necessary for a quantitative estimate of accuracy from Marker Mapper and Aruco. The data above suggest that the pose estimate has reasonably low error over a short peroid of time, but global position is inaccurate from Marker Mapper. Further investigation is necessary to determine if this is because of the position estimate from Aruco or errors from generating the marker map.
    ...
    \begin{figure}[H]
      \centering
      \includegraphics[width=.98\linewidth]{./images/marker_mapper_hallway_misid.png}
      \caption{Tags 7 and 21 were misidentified while generating this Marker Map.}
      \label{fig:misid_marker_mapper}
    \end{figure}
...
\section{System Specification} \label{section:specs}

  \subsection{Design Criteria} \label{section:design_criteria}
  ...
  \subsection{Implementation Details}
    ...
    \subsubsection{NavX Processing}

      % describe our final methods for what we're actually doing with the navx data
      In this section we describe our final methods for incorporating the accelerometer readings and fused yaw readings from the NavX IMU. In short, the final process is to apply calibration parameters, rotate the accelerometer readings into the robot base frame, apply drift and bias compensation, the feed these readings into our extended kalman filter. Experimental results on each of these steps are presented above in Sections \ref{section:imu_calibration}, \ref{section:drift_bias}, and \ref{section:zero_velocity_updates}, but here we describe the final process in greater detail.

      The first step is to apply the calibration values, which is a straightforward application of Equation \ref{eq:accelerometer_calibration}. These parameters are determined a priori by the process described in Section \ref{section:imu_calibration}. $a^S$ is a 3 by 1 vector of the raw accelerometer readings, and $a^O$ is the same vector now calibrated. This calibration process accounts for axis misalignment, scaling, and bias errors.

      \begin{equation} \label{eq:accelerometer_calibration}
        a^O = T^aK^a(a^S+b^a) = \begin{bmatrix}
      ...
      After calibration, we account for the NavX not being mounted in the same orientation as the robot's base frame. We would like the positive accelerometer readings in X to point ``forward'' in the robot frame, and the positive readings in Z to point upwards. To achieve this, we assume the robot is stationary for the first \SI{5}{\second} of being turned on, and record the accelerometer readings during this period. We then use the Equation \ref{eq:base_frame_rotation} to find the rotation that rotates those measured accelerations into the expected acceleration in the robot base frame. This expected acceleration in the robot frame is $[0,0,1]$. We cannot infer the yaw of the robot from this process, so we assume the yaw offset between the NavX and the robot is given to use a priori.

      \begin{align}
        R &= I + [v]_{\times} + {[v]_{\times}}^2\frac{1}{1+c}  \label{eq:base_frame_rotation}\\
        [v]_{\times} &= \begin{bmatrix} 0 & -v_3 & v_2 \\ v_3 & 0 & -v_1 \\ -v_2 & v_1 & 0 \\ \end{bmatrix} \nonumber\\
        v &= a\times b \nonumber\\
        c &= a\cdot b \nonumber
      \end{align}

      After calibration and base frame rotation, a static detector was applied to all of the calibrated and base-frame-rotated data to classify the static and dynamic intervals. In this static detector, we used $\varsigma^6$ as the threshold and 80 samples as the sliding window, where $\varsigma$ is the magnitude of the variance of an example static period (Equation \ref{eq:threshold}). We chose $\varsigma^6$ experimentally by raising the power, which lowers the threshold, until the intervals were sufficiently static.

      \begin{equation} \label{eq:threshold}
        \varsigma = \sqrt{[\text{var}(a^t_x)]^2+[\text{var})(a^t_y)]^2+[\text{var})(a^t_z)]^2}
      \end{equation}

      %With this static detector, we can now compensate for the bias and drift in bias of the accelerometer. We model the accelerometer drifted as linear over time and assume that every dynamic interval had the same drifting behavior as its neighbor static interval. We paired every dynamic interval with its previous static interval. We characterized the drift rate $d_i$ on each static interval $I_i$ using the Equation \ref{eq:drift_comp}. $Avg(t_o, t_f)$ is a function that takes a start time $t_o$ and an end time $t_f$ and returns the average accelerometer value during the range $(t_o,t_f)$.

      %\begin{equation} \label{eq:drift_comp}
        %d_i = \frac{Avg(t_f-t_{window},t_f) - Avg(t_o,t_o+t_{window})}{(t_f-t_o-t_{window})}
      %\end{equation}

      %Then, we applied drift compensation on every data points in each interval. The value $v^j_i$ added on each data point can be calculated as $v^j_i=-d_ij\delta t$.

      We next applied zero velocity update on the data. Zero velocity update removed bias on every data sample by assuming that the bias of a data sample is the same as the mean bias of the most recent static interval. Zero velocity updates also involve resetting the velocity estimate to 0, which could be implemented as a hard-reset or a probabilistic EKF measurement update with near zero variance to indicate high confidence.

      Lastly, we applied clockwise rotation matrix $R(\theta)$ on every accelerometer data to transfer the direction of accelerometer data into the world frame, where $\theta$ is the yaw reported by the NavX function \texttt{GetYaw()}.
