...
\section{Experimental Results} \label{experiments}
  ...
  \subsection{Characterising Drift and Bias in the Accelerometer}
    ...
    In this section, we demonstrate that naive double integration is inaccurate, but that with calibration, base frame rotation, and zero velocity updates we can acheive accurate position information from the NavX IMU.
    ...
    \subsubsection{Measuring the drift and bias in the accelerometer}
      ...
      We then wondered that whether the duration of motion influences the amount of drift, so we performed an another experiment. We drove the robot in a circle, stop for 9 seconds,  drove the robot in 2 circles, stop for 9 second, so on, until the robot drove for 5 circles. We then collected accelerometer data, yaw angle, and temperature. We studied the drifting behavior and biases by observing the mean accelerometer value in several static intervals. Those values are shown in Figure \ref{fig:static_means}.
      ...
      The NavX accelerometer drifted a lot among several static intervals. It seemed like motions affect drift a lot. However, the drifted setting values didn't seem to have a trend over motioness.

      Since temperature could also be a factor to affect accelerometer values, we compared the temperature with accelerometer values in static intervals over time.
      ...
      Shown in Figure \ref{fig:temperature}, we have seen that the temperature overall decreased over time. However, the biases in static intervals over time didn't show a specific trend. Temperature seemed to not have a straightforward relationship with accelerometer drift.

      Overall, we figured out that the accelerometer drifted way much in motion than being stationary.Moreover, it drifted randomly and did not have a trend. Also, temperature seemed not having an obvious association with drift trend.

    \subsubsection{Drift Compensation}

    % then, explain what we tried to do to compensate for drift, and why it didn't really fix it
    We integrated the calibrated accelerometer data with yaw angles of the circle test (refer back to section 5.3.1) to visualize velocity and displacement of our robot. We can easily see in figure xxx and yyy that drift affect the accuracy of velocity and displacement a lot.

      \begin{figure}[H]
        \centering
        \includegraphics[width=1\linewidth]{./images/calibrated_velocity.png}
        \caption{Velocity as dervied by integrating the calibrated accelerometer measurements.}
        \label{fig:calibrated_velocity}
      \end{figure}

      \begin{figure}[H]
        \centering
        \includegraphics[width=1\linewidth]{./images/calibrated_displacement.png}
        \caption{Displacement as dervied by twice-integrating the calibrated accelerometer measurements.}
        \label{fig:calibrated_displacement}
      \end{figure}

      We came up with two algorithm to fix the drifting issue. The method is to calculate the drift rates of  two sequential static intervals, and apply drift compensation on the first static interval and the following dynamic intervals. This method is a offline method since it has to go over time and determine the drift rate between two static intervals. The online method is to calculate the drift rate within a static interval and project this drifting behavior on both the static interval and the following dynamic interval. Both of the method had no obvious effect on fixing the drift in velocity.

      \begin{figure}[H]
        \centering
        \includegraphics[width=1\linewidth]{./images/online_drift_compensation.png}
        \caption{Compensate for drift by ...}
        \label{fig:online_drift_compensation}
      \end{figure}

      \begin{figure}[H]
        \centering
        \includegraphics[width=1\linewidth]{./images/offline_drift_compensation.png}
        \caption{Compensate for drift by ...}
        \label{fig:offline_drift_compensation}
      \end{figure}

      Although the fixing was trivial in NavX's accelerometer, we still used drift compensating in our IMU localization algorithm in case it may work greatly on other IMUs.

    \subsubsection{Zero Velocity Updates}

    % talk about how zero velocity udpates improve double integration/position

    Looked at the drift compensated velocity, clearly there were still biases inside of the data. We decided to perform zero velocity update algorithm on the data. When the algorithm detected the robot being stationary, it calculated and recorded the bias in the static interval, removed the bias for the static interval and the following dynamic interval and then set the current velocity to be zero.
  ...
  \subsection{Comparing Our IMU Localization to the NavX API}
    ...
    Applied pure double integration of raw IMU data on the circle test [REF] to get locations of the robot (figure x), we determined that it was not sufficient to meet our accuracy standard. Moreover, NavX location API (figure y) also didn't meet our standard.

    In order to improve the accuracy of IMU localization, we explored and experimented different methods (IMU calibration, etc.) on data of the circle test [REF]. Since we didn't have a ground truth to measure the accuracy of our best method, we compared it with the NavX's get location API.
...
\section{System Specification} \label{section:specs}
  ...
  \subsection{Implementation Details}
    ...
    \subsubsection{Accelerometer Processing}
      ...
      Our method started with a separate prerequisite. It requested all of the accelerometer data to be calibrated and base-frame-rotated. The NavX was calibrated using the method in section xxx. The calibration corrected the misalignment, scaling and biases in accelerometer. After calibration, we mounted the NavX on the robot and collected few seconds of accelerometer data while the NavX is static. Using those data, we calculated the base frame rotation matrix. The matrix transferred the data from robot frame to the world frame.
      After all the prerequisites were done, an static detector (mentioned in section xxx) was applied to all of the calibrated and base-frame-rotated data to classify the static and dynamic intervals (Figure D). In this static detector, we used $\zeta^6$ as the threshold and 80 samples as the sliding window.
      (figure D)
      Assumed the accelerometer drifted linearly and every dynamic interval had the same drifting behavior as its neighbor static interval. We paired every dynamic interval $DI_i$ with its previous static interval $SI_i$ as $I_i$, for $i\in N$ static intervals. We characterized the drift rate $d_i$ on $I_i$ using the function below. $S(I)$ is a function that takes an interval and returns the start time of the interval, where $E(I$) returns the end time of the interval. $Avg(t_o, t_f)$ is a function that takes a start time $t_o$ and an end time $t_f$ and returns the average accelerometer value during the range $(t_o,t_f)$.
      ...
      We next applied zero-velocity update on the data. Zero velocity update removed bias on every data sample by assuming that the bias of a data sample in an interval $I_i$ is the same as the bias of other data samples in that interval. Bias $b_i$ in interval $I_i$ can be characterized as

      $$b_i=Avg(S(SI_i), E(SI_i))$$

      We then applied clockwise rotation matrix $R(\theta)$ on every accelerometer data to transfer the direction of accelerometer data into the world frame. Finally, we double integrated the accelerometer data to get location of the robot.
