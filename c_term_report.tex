...
\section{Experimental Results} \label{experiments}
  ...
  \subsection{IMU Calibration}
    ...
    \begin{figure}[H]
      ...
      \caption{Integration of the gyroscope readings in the Y Axis. Method 1 is one-shot rotation, Method 2 is incremental rotation.}
  ...
  \subsection{The Doppler Effect on Ultrasonic}

  \subsection{Limits of the ADC and DAC on the PSoC 5LP}
  ...
  \subsection{Characteristics of Piezo Transducers}
    ...
    % FIXME
		5 db loss over 15 feet. At 17 feet away, we saw best signal strength at 22.5kHz. The signal db is about 6.2 dB SNR! Noise floor is -58.7dB? what does that mean? If we measure a 1 volt RMS (353.5 mv peak-to-peak) sine wave, and the oscilloscope says -11 db? What does that mean?

    Emitting sine wave works just as well as square wave.

	\subsection{Co-Processors for Image Processing}

    We evaluated the Raspberry PI 3 and the NVidia TK1. Neither was capable of recording 480p30, but either was good enough to read and process 480p30 just not save to video. % FIXME: more on what each platform is capable of
    ...
    % compare inidivual points in frames
    % compare covariance after kalman filtering

	\subsection{Update rate of localization from ArUco Tags}
	...
	\subsection{Evaluating NavX Location API}
...
\section{System Specification} \label{specs}
  ...
  \subsection{?? stupid title}\label{section:system_spec}
